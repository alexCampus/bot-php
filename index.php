...
if($method == 'POST'){
	...
	if (count($speech) === 1) {
		...
		foreach ($speech as $key => $value) {
			...
			array_push($response->fulfillmentMessages[]['text']['text'], [$speech[$i]->nom . ' est dans le département ' . $speech[$i]->codeDepartement . ' et il y a ' . number_format($speech[$i]->population) . ' habitants.']);
